class AgendamentoController {
  ...
  async dashboard(req, res) {
    ...
    const preMedicoId = req.query.medicoId ? Number(req.query.medicoId) : undefined;
    const preData = req.query.data || undefined;
    const preHora = req.query.hora || undefined;
    const dataHora = (preData && preHora) ? `${preData}T${preHora}` : '';
    const data = { medicoId: preMedicoId, dataHora };
    res.render('agendamento', { title: 'Agendamento', medicos, pessoas, consultas, data, errors: {}, sucesso: null });
  }

  async agendar(req, res) {
    const payload = { medicoId: Number(req.body.medicoId), pacienteId: Number(req.body.pacienteId), dataHora: req.body.dataHora };
    const medicos = await this.medicoService.listar();
    const pessoas = await this.pessoaService.listar();
    const consultas = await this.agendamentoService.listarConsultas();
    ...
    const consulta = await this.agendamentoService.agendarConsulta(payload.medicoId, payload.pacienteId, payload.dataHora);
    if (!consulta) {
